...
public class TestDonationCreateMembershipDailyPageNew {
	...
	@Test
	public void testDonationCreateMembershipDailyPageNew() throws Exception {
		...
		selenium.type("id=page_name", selenium.getEval("'Production_GP_MFRP_Daily_ACVV'+Math.floor(Math.random()*10000)"));
		verifyTrue(selenium.isTextPresent("Public Title:"));
		assertTrue(selenium.isElementPresent("id=public_title"));
		selenium.type("id=public_title", selenium.getEval("'Production_GP_MFRP_Daily_ACVV'+Math.floor(Math.random()*10000)"));
		verifyTrue(selenium.isTextPresent("Page Url:"));
		assertTrue(selenium.isElementPresent("id=slugInput-textBox-value"));
		selenium.type("id=slugInput-textBox-value", selenium.getEval("'production_gp_mfrp_daily_acvv_slug'+Math.floor(Math.random()*10000)"));
		...
		verifyTrue(selenium.isTextPresent("Membership"));
		assertTrue(selenium.isElementPresent("id=type-membership-label"));
		selenium.click("id=type-membership-label");
		assertEquals("on", selenium.getValue("id=type-membership"));
		verifyTrue(selenium.isTextPresent("Setup a Membership Contribution Page"));
		verifyTrue(selenium.isTextPresent("Membership Label"));
		assertTrue(selenium.isElementPresent("name=membership_label"));
		selenium.type("name=membership_label", "Production Membership-Daily Page");
		verifyTrue(selenium.isTextPresent("Membership Cost:"));
		assertTrue(selenium.isElementPresent("name=membership_amt"));
		selenium.type("name=membership_amt", "10");
		verifyTrue(selenium.isTextPresent("Lifetime Membership:"));
		assertTrue(selenium.isElementPresent("id=membership_is_lifetime"));
		selenium.select("id=membership_is_lifetime", "label=No");
		verifyTrue(selenium.isTextPresent("Membership Interval Length:"));
		assertTrue(selenium.isElementPresent("id=membership_interval_number"));
		selenium.type("id=membership_interval_number", "1");
		assertTrue(selenium.isElementPresent("id=membership_interval_unit"));
		selenium.select("id=membership_interval_unit", "label=day(s)");
		...
		selenium.type("name=thanks_email_from_name", "Production Automated GP Membership-Daily Donation Testing");
		verifyTrue(selenium.isTextPresent("Subject:"));
		assertTrue(selenium.isElementPresent("name=thanks_email_subject"));
		selenium.type("name=thanks_email_subject", "Production GP Membership-Daily Donation Tests!!!");
