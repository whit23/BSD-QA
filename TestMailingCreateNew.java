...
public class TestMailingCreateNew {
	...
	@Test
	public void testMailingCreateNew() throws Exception {
		...
		verifyTrue(selenium.isElementPresent("id=email-create-new"));
		selenium.click("id=email-create-new");
		verifyEquals("Maillist Edit BSD Control Panel", selenium.getTitle());
		selenium.type("name=mailing_name", selenium.getEval("'1-Production_GP_Mailing_New'+Math.floor(Math.random()*100000)"));
		verifyTrue(selenium.isTextPresent("exact:*Mailing Template"));
		selenium.select("id=mailing_template_id", "label=American Red Cross");
		selenium.click("css=option[value=\"8\"]");
		verifyTrue(selenium.isTextPresent("Description"));
		selenium.type("id=description", "Production Guinea Pig Mailing Test...");
		...
		selenium.type("id=subject", "Subject: Production Guinea Pig Mailing Test...");
		selenium.click("id=wysiwygedit-label-body_html");
		selenium.type("id=body_html", "Body: Production Guinea Pig Mailing Test...");
		selenium.click("id=wysiwygedit-label-body_html");
		...
		assertTrue(selenium.isTextPresent("1-Production_GP_Mailing_New"));
	}
