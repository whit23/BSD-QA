...
public class TestMailingCreateTWIGOldViaCloning {
	...
	@Test
	public void testMailingCreateTWIGOldViaCloning() throws Exception {
		...
		selenium.type("name=simpleQuery", "1-Production_GP_Old_Style_TWIG (ALL Tags) Mailing98430");
		...
		selenium.type("name=mailing_name", selenium.getEval("'1-Production_GP_Old_Style_TWIG (ALL Tags) Mailing'+Math.floor(Math.random()*100000)"));
		selenium.type("id=description", "Production Guinea Pig TWIG Old Style Mailing Test...");
		...
		for (int second = 0;; second++) {
			if (second >= 60) fail("timeout");
			try { if (selenium.isTextPresent("glob:*1-Production_GP_Old_Style_TWIG (ALL Tags) Mailing*")) break; } catch (Exception e) {}
			Thread.sleep(1000);
		}

		selenium.click("id=draft-tab-link");
		selenium.selectWindow("null");
		verifyTrue(selenium.getText("css=span[title=\"1-Production_GP_Old_Style_TWIG (ALL Tags) Mailing11827\"]").matches("^[\\s\\S]*1-Production_GP_Old_Style_TWIG \\(ALL Tags\\) Mailing11827[\\s\\S]*$"));
	}
